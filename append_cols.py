tableB_cols = {
    "2009": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Abundance_#",
        "Callitriche_hermaphroditica",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hypericum_ellipticum",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Polygonam_amphibium",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_sp.",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithaphora_sp."
    ],
    "2010": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton__zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithaphora_sp."
    ],
    "2011": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton__zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithaphora_sp.",
        "Floating_Filamentous_Algae"
    ],
    "2012": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton__zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithaphora_sp.",
        "Floating_Filamentous_Algae"
    ],
    "2013": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhia",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithaphora_sp.",
        "Unnamed:_38"
    ],
    "2014": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Alisma_gramineum",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Zannichellia_palustris",
        "Pithophora_sp."
    ],
    "2015": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Alisma_gramineum",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Polygonum_amphibium",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Riccia_fluitans",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithophora_sp."
    ],
    "2016": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Alisma_gramineum",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Riccia_fluitans",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Stuckenia_vaginata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Zannichellia_palustris",
        "Pithophora_sp.",
        "Unnamed:_42",
        "Unnamed:_43",
        "Unnamed:_44"
    ],
    "2017": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Polygonum_amphibium",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithophora_sp."
    ],
    "2018": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Alisma_gramineum",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Polygonum_amphibium",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithophora_sp."
    ],
    "2019": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Alisma_gramineum",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithophora_sp."
    ],
    "2020": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Alisma_gramineum",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_sibiricum",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Riccia_fluitans",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Utricularia_vulgaris",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithophora_sp."
    ],
    "2021": [
        "Sample_Point_(SP)",
        "Latitude",
        "Longitude",
        "Rake_Abundance",
        "Rake_Abundance_#",
        "Alisma_gramineum",
        "Ceratophyllum_demersum",
        "Chara_vulgaris",
        "Elodea_sp.",
        "Fontinalis_sp.",
        "Heteranthera_dubia",
        "Hydrocharis_morsus-ranae",
        "Lemna_minor",
        "Lemna_trisulca",
        "Megalodonta_beckii",
        "Myriophyllum_spicatum",
        "Najas_flexilis",
        "Najas_guadalupensis",
        "Nitella_flexilis",
        "Nitellopsis_obtusa",
        "Nuphar_variegata",
        "Nymphaea_odorata",
        "Potamogeton_amplifolius",
        "Potamogeton_crispus",
        "Potamogeton_foliosus",
        "Potamogeton_gramineus",
        "Potamogeton_illinoensis",
        "Potamogeton_praelongus",
        "Potamogeton_pusillus",
        "Potamogeton_richardsonii",
        "Potamogeton_zosteriformis",
        "Ranunculus_trichophyllus",
        "Spirodela_polyrhiza",
        "Stuckenia_pectinata",
        "Utricularia_sp.",
        "Vallisneria_americana",
        "Wolffia_columbiana",
        "Pithophora_sp."
    ]
}

years = [2009, 2010, 2011, 2012, 2013, 2014,
         2015, 2016, 2017, 2018, 2019, 2020, 2021]

for year in years:
    yr = str(year)
    print(f"ORIGINAL LENGTH OF {year}:", len(tableB_cols[yr]))
    num_cols = len(tableB_cols[yr])
    if num_cols < 45:
        for i in range(45-num_cols):
            tableB_cols[yr].append(None)
    print(f"NEW LENGTH OF {year}:", len(
        tableB_cols[yr]), "\n", tableB_cols[yr])

print("=======================================\n", "tableB_cols = ", tableB_cols)
